...
class UIConsola:
    ...
    def ejecutar_turno_de_la_casa(self):
        def ejecutar_turno_de_la_casa(self):
            print("\n--- Turno de la Casa ---")
            self.mostrar_manos(self.cupier.mano, self.jugador.mano)
            
            while not self.blackjack.casa_gano() and self.casa_puede_pedir():
                print("La casa decide pedir otra carta.")
                self.repartir_carta_a_la_casa()
                self.mostrar_manos(self.cupier.mano, self.jugador.mano)
                
            if not self.casa_puede_pedir():
                print("La casa decide plantarse.")
